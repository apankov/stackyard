...
stack_missing_files() {
  local s="$1"
  # compose.yaml обязателен только у стека, у которого есть СВОИ контейнеры.
  #
  # На этой машине статику и PHP раздают платформенные nginx и php-fpm, а
  # docroot'ы лежат в общем, всегда смонтированном $Platform_Vhosts_Dir. Поэтому
  # сайт вроде design.12devs.com — это каталог с одним только vhost'ом, и
  # требовать от него compose-файл значило бы заставить заводить пустышку
  # `services: {}` ради проверки. Объявлением служит наличие каталога: есть
  # nginx/ — стек живёт на платформенных контейнерах и своих не имеет.
  if [ ! -f "$(stack_compose_file "$s")" ] && [ ! -d "$(stack_vhost_dir "$s")" ]; then
    printf 'stacks/%s/compose.yaml (или nginx/ — если стек живёт на платформенных контейнерах)\n' "$s"
  fi
