...
magic_hex=$(od -An -v -tx1 -N16 "$FILE" | tr -d ' \n')
case "$magic_hex" in

  1f8b*)                        KIND=sqlite_gz ;;    # gzip
  53514c69746520666f726d6174*)  KIND=sqlite_plain ;; # "SQLite format"
  # Всё остальное — формат СУБД. Какой именно, знает её поставщик; SQLite
  # опознан выше потому, что не принадлежит ни одному из них: его объявляют
  # сами стеки через Backup_Sqlite.
  *)                            KIND="db:$(db_hook detect "$FILE" 2>/dev/null || echo unknown)" ;;
esac
